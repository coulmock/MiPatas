...
export const GeminiService = {
  ...
  generateStaticResponse(messages: ChatMessage[], petContext?: Partial<Pet>): string {
    ...
    } else if (lastUserMsg.includes('ladra') || lastUserMsg.includes('tira') || lastUserMsg.includes('muerde') || lastUserMsg.includes('solo') || lastUserMsg.includes('miedo') || lastUserMsg.includes('salta')) {
      specificTopic = `\n\n🐾 **Pauta orientativa de comportamiento ("Educa & Entiende"):**\n\n🔍 **Hipótesis posibles:** La conducta descrita puede estar vinculada a picos de sobreexcitación, falta de estimulación olfativa o inseguridad ante estímulos ambientales.\n\n👀 **Qué observar:** Evalúa su postura corporal (tensión muscular, posición de la cola, jadeo sin calor) y los detonantes exactos.\n\n🛠️ **Acciones inmediatas:**\n1. Reduce el estrés ambiental y fomenta 15 min diarios de olfato en el parque.\n2. Evita castigos o tirones de correa que aumenten la frustración.\n3. Ofrece mordedores deshidratados o juguetes tipo Kong para autorrelajación.\n\n🗓️ **Plan progresivo (7 días):** Días 1-2: observación y rebaja de estímulos. Días 3-5: introducción de ejercicios de calma ("A tu sitio"). Días 6-7: consolidación con refuerzo positivo.\n\n🩺 *Nota de seguridad: Si la conducta incluye reactividad agresiva con mordiscos o es un cambio repentino, consulta con tu veterinario para descartar dolor y acude a un etólogo o educador canino profesional.*`;
    }
